...
```
 <processor name="DataParser">
	<parameter>
		<data field="bit.48"/>
		<items>
			<item len="1"  field="totalOutstandingBills"/>
			<item repeat="totalOutstandingBills">
				<item len="12"  field="penaltyToBePaid"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="6"  field="periodBillToBePaid"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="8"  field="dueDate"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="8"  field="meterReadingDate"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="12"  field="rpTag"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="1"  field="billIncentive"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="10"  field="vat"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="8"  field="slalwbp1"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="8"  field="sahlwbp1"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="8"  field="slalwbp2"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="8"  field="sahlwbp2"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="8"  field="previousSlakvarh"/>
			</item>
			<item repeat="totalOutstandingBills">
				<item len="8"  field="currentSlakvarh"/>
			</item>
		</items>
	</parameter>
</processor>
...
```
repeat artinya value akan masuk ke fields penaltyToBePaidsecara berulang sebanyak putaran totalOutstandingBills
...
```
Jika ingin ambil index 3 and take the rest tidak perlu isi to="" di func substr
...
## FEE ##
Setup m_biller dulu, lalu di m_produt_biller , dealer di m_biller adalah channel code di m_module

	1. Fee auto setup tinggla di route ke fee contoh {fee,core,biller}
	2. Fee.xml di setup di m_paramter dengan key FEE_PROCESSOR
	
fee.xml itu biasanya buat ngeset fitur2 yang ga dapet kiriman billerCode productCode dari depan. billerCode dan productCode adalah field yang dipakai mgate untuk otomatis link ke m_fee
)
